"""
Lumynax Frontier Dbrx Instruct 132B Gguf — LumynaX quickstart (clone & run).

This loads the GGUF that ships with this repo. No upstream HF call needed
once you've done `hf download AbteeXAILab/lumynax-frontier-dbrx-instruct-132b-gguf`.
...
PRIMARY = HERE / r"dbrx-instruct.Q2_K.gguf"
...
def main():
    ...
    if not PRIMARY.exists():
        ...
        print(f"[lumynax] run: hf download AbteeXAILab/lumynax-frontier-dbrx-instruct-132b-gguf --local-dir <dir> first.", file=sys.stderr)
        sys.exit(2)
