...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Infused Granite 3.3 8B Instruct GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Infused Granite 3.3 8B Instruct GGUF</h1>
...
> `AbteeXAILab/lumynax-infused-granite33-8b-gguf` is a **complete LumynaX release package**: model artifact, `quickstart.py`, `requirements.txt`, `release_export_manifest.json`, `checksums.sha256`, license notice, and optional Ollama / Space scaffolds shipped as **one downloadable contract**. Clone whole, verify by checksum, and run close to the data it serves.
...
```bash
hf download AbteeXAILab/lumynax-infused-granite33-8b-gguf --local-dir lumynax-infused-granite33-8b-gguf
cd lumynax-infused-granite33-8b-gguf
...
llm = Llama(model_path="granite-3.3-8b-instruct-Q4_K_M.gguf", n_ctx=8192, n_threads=8, verbose=False)
...
```bash
llama-cli -m "granite-3.3-8b-instruct-Q4_K_M.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-infused-granite33-8b-gguf -f ollama/Modelfile
ollama run lumynax-infused-granite33-8b-gguf
```

**Verify integrity before launch:**

```bash
sha256sum "granite-3.3-8b-instruct-Q4_K_M.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "granite-3.3-8b-instruct-Q4_K_M.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Infused Granite 3.3 8B Instruct GGUF` |
| Repository | `AbteeXAILab/lumynax-infused-granite33-8b-gguf` |
| Family | `granite` |
...
| Field | Value |
| --- | --- |
| Primary | `granite-3.3-8b-instruct-Q4_K_M.gguf` |
| Weight size | `4.60 GB` |
| Parameters | `8.0B` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `ibm-granite/granite-3.3-8b-instruct` |
...
| Primary artifact | `granite-3.3-8b-instruct-Q4_K_M.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-infused-granite33-8b-gguf/
├── README.md                       # this card
...
└── granite-3.3-8b-instruct-Q4_K_M.gguf# primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Infused Granite 3.3 8B Instruct GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
