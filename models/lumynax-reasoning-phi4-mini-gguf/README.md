...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Reasoning Phi-4 Mini GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Reasoning Phi-4 Mini GGUF</h1>
...
```bash
hf download AbteeXAILab/lumynax-reasoning-phi4-mini-gguf --local-dir lumynax-reasoning-phi4-mini-gguf
cd lumynax-reasoning-phi4-mini-gguf
...
llm = Llama(model_path="Phi-4-mini-reasoning-Q4_K_M.gguf", n_ctx=8192, n_threads=8, verbose=False)
...
```bash
llama-cli -m "Phi-4-mini-reasoning-Q4_K_M.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-reasoning-phi4-mini-gguf -f ollama/Modelfile
ollama run lumynax-reasoning-phi4-mini-gguf
```

**Verify integrity before launch:**

```bash
sha256sum "Phi-4-mini-reasoning-Q4_K_M.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "Phi-4-mini-reasoning-Q4_K_M.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Reasoning Phi-4 Mini GGUF` |
| Repository | `AbteeXAILab/lumynax-reasoning-phi4-mini-gguf` |
| Family | `phi` |
...
| Field | Value |
| --- | --- |
| Primary | `Phi-4-mini-reasoning-Q4_K_M.gguf` |
| Weight size | `2.32 GB` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `microsoft/Phi-4-mini-reasoning` |
| Source | `unsloth/Phi-4-mini-reasoning-GGUF` |
| License | `mit` |
...
| Primary artifact | `Phi-4-mini-reasoning-Q4_K_M.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-reasoning-phi4-mini-gguf/
├── README.md                       # this card
...
└── Phi-4-mini-reasoning-Q4_K_M.gguf# primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Reasoning Phi-4 Mini GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
