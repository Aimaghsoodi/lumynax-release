...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Coder CodeLlama 70B Instruct GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Coder CodeLlama 70B Instruct GGUF</h1>
...
> `AbteeXAILab/lumynax-coder-codellama-70b-instruct-gguf` is a **complete LumynaX release package**: model artifact, `quickstart.py`, `requirements.txt`, `release_export_manifest.json`, `checksums.sha256`, license notice, and optional Ollama / Space scaffolds shipped as **one downloadable contract**. Clone whole, verify by checksum, and run close to the data it serves.
...
```bash
hf download AbteeXAILab/lumynax-coder-codellama-70b-instruct-gguf --local-dir lumynax-coder-codellama-70b-instruct-gguf
cd lumynax-coder-codellama-70b-instruct-gguf
...
llm = Llama(model_path="codellama-70b-instruct.Q4_K_M.gguf", n_ctx=16384, n_threads=8, verbose=False)
...
```bash
llama-cli -m "codellama-70b-instruct.Q4_K_M.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-coder-codellama-70b-instruct-gguf -f ollama/Modelfile
ollama run lumynax-coder-codellama-70b-instruct-gguf
```

**Verify integrity before launch:**

```bash
sha256sum "codellama-70b-instruct.Q4_K_M.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "codellama-70b-instruct.Q4_K_M.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Coder CodeLlama 70B Instruct GGUF` |
| Repository | `AbteeXAILab/lumynax-coder-codellama-70b-instruct-gguf` |
| Family | `codellama` |
...
| Field | Value |
| --- | --- |
| Primary | `codellama-70b-instruct.Q4_K_M.gguf` |
| Weight size | `—` |
| Parameters | `70B` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `codellama/CodeLlama-70b-Instruct-hf` |
| Source | `codellama/CodeLlama-70b-Instruct-hf` |
| License | `other` |
...
| Primary artifact | `codellama-70b-instruct.Q4_K_M.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-coder-codellama-70b-instruct-gguf/
├── README.md                       # this card
...
└── codellama-70b-instruct.Q4_K_M.gguf# primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Coder CodeLlama 70B Instruct GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
