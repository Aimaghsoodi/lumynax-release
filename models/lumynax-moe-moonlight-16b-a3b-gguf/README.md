...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX MoE Moonlight 16B A3B GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX MoE Moonlight 16B A3B GGUF</h1>
...
> `AbteeXAILab/lumynax-moe-moonlight-16b-a3b-gguf` is a **complete LumynaX release package**: model artifact, `quickstart.py`, `requirements.txt`, `release_export_manifest.json`, `checksums.sha256`, license notice, and optional Ollama / Space scaffolds shipped as **one downloadable contract**. Clone whole, verify by checksum, and run close to the data it serves.
...
```bash
hf download AbteeXAILab/lumynax-moe-moonlight-16b-a3b-gguf --local-dir lumynax-moe-moonlight-16b-a3b-gguf
cd lumynax-moe-moonlight-16b-a3b-gguf
...
llm = Llama(model_path="lumynax-moe-moonlight-16b-a3b-iq4_xs.gguf", n_ctx=8192, n_threads=8, verbose=False)
...
```bash
llama-cli -m "lumynax-moe-moonlight-16b-a3b-iq4_xs.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-moe-moonlight-16b-a3b-gguf -f ollama/Modelfile
ollama run lumynax-moe-moonlight-16b-a3b-gguf
```

**Verify integrity before launch:**

```bash
sha256sum "lumynax-moe-moonlight-16b-a3b-iq4_xs.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "lumynax-moe-moonlight-16b-a3b-iq4_xs.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX MoE Moonlight 16B A3B GGUF` |
| Repository | `AbteeXAILab/lumynax-moe-moonlight-16b-a3b-gguf` |
| Family | `lumynax` |
...
| Field | Value |
| --- | --- |
| Primary | `lumynax-moe-moonlight-16b-a3b-iq4_xs.gguf` |
| Weight size | `8.14 GB` |
| Parameters | `16.0B / 3.0B active` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `moonshotai/Moonlight-16B-A3B-Instruct` |
| Source | `mmnga/Moonlight-16B-A3B-Instruct-gguf` |
| License | `apache-2.0` |
...
| Primary artifact | `lumynax-moe-moonlight-16b-a3b-iq4_xs.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-moe-moonlight-16b-a3b-gguf/
├── README.md                       # this card
...
└── lumynax-moe-moonlight-16b-a3b-iq4_xs.gguf# primary artifact
```
...
```text
You are LumynaX operating from the LumynaX MoE Moonlight 16B A3B GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
