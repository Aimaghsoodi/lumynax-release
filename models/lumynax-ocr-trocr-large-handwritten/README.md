...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX OCR TrOCR Large Handwritten release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX OCR TrOCR Large Handwritten</h1>
...
> `AbteeXAILab/lumynax-ocr-trocr-large-handwritten` is a **complete LumynaX release package**: model artifact, `quickstart.py`, `requirements.txt`, `release_export_manifest.json`, `checksums.sha256`, license notice, and optional Ollama / Space scaffolds shipped as **one downloadable contract**. Clone whole, verify by checksum, and run close to the data it serves.
...
```bash
hf download AbteeXAILab/lumynax-ocr-trocr-large-handwritten --local-dir lumynax-ocr-trocr-large-handwritten
cd lumynax-ocr-trocr-large-handwritten
...
tokenizer = AutoTokenizer.from_pretrained("AbteeXAILab/lumynax-ocr-trocr-large-handwritten", trust_remote_code=True)
model = AutoModelForCausalLM.from_pretrained("AbteeXAILab/lumynax-ocr-trocr-large-handwritten", device_map="auto", trust_remote_code=True)
...
| Field | Value |
| --- | --- |
| Release | `LumynaX OCR TrOCR Large Handwritten` |
| Repository | `AbteeXAILab/lumynax-ocr-trocr-large-handwritten` |
| Family | `trocr` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `microsoft/trocr-large-handwritten` |
...
```text
lumynax-ocr-trocr-large-handwritten/
├── README.md                       # this card
...
```text
You are LumynaX operating from the LumynaX OCR TrOCR Large Handwritten package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
