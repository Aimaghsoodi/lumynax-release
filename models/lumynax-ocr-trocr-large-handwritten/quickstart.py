"""
Lumynax Ocr Trocr Large Handwritten — LumynaX quickstart (clone & run, multimodal safetensors).
...
def main():
    ...
    if not (HERE / "model.safetensors").exists():
        ...
        print(f"[lumynax] run: hf download AbteeXAILab/lumynax-ocr-trocr-large-handwritten --local-dir <dir> first.", file=sys.stderr)
        sys.exit(2)
