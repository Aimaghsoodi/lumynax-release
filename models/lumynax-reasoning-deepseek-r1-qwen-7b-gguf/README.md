...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Reasoning DeepSeek R1 Distill Qwen 7B GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Reasoning DeepSeek R1 Distill Qwen 7B GGUF</h1>
...
<p align="center"><kbd>Quality: <b>3/5</b></kbd> &middot; <kbd>Lightweight: <b>0/5</b></kbd> &middot; <kbd>Sovereignty: <b>3/5</b></kbd> &middot; <kbd>Tools: <b>yes</b></kbd> &middot; <kbd>JSON: <b>yes</b></kbd> &middot; <kbd>Context: <b>8192 tok</b></kbd></p>

---

## 📦 Executive Summary

> `AbteeXAILab/lumynax-reasoning-deepseek-r1-qwen-7b-gguf` is a **complete LumynaX release package**: model artifact, `quickstart.py`, `requirements.txt`, `release_export_manifest.json`, `checksums.sha256`, license notice, and optional Ollama / Space scaffolds shipped as **one downloadable contract**. Clone whole, verify by checksum, and run close to the data it serves.
...
```bash
hf download AbteeXAILab/lumynax-reasoning-deepseek-r1-qwen-7b-gguf --local-dir lumynax-reasoning-deepseek-r1-qwen-7b-gguf
cd lumynax-reasoning-deepseek-r1-qwen-7b-gguf
...
llm = Llama(model_path="DeepSeek-R1-Distill-Qwen-7B-Q4_K_M.gguf", n_ctx=8192, n_threads=8, verbose=False)
...
```bash
llama-cli -m "DeepSeek-R1-Distill-Qwen-7B-Q4_K_M.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-reasoning-deepseek-r1-qwen-7b-gguf -f ollama/Modelfile
ollama run lumynax-reasoning-deepseek-r1-qwen-7b-gguf
```
...
```powershell
Get-FileHash -Algorithm SHA256 "DeepSeek-R1-Distill-Qwen-7B-Q4_K_M.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Reasoning DeepSeek R1 Distill Qwen 7B GGUF` |
| Repository | `AbteeXAILab/lumynax-reasoning-deepseek-r1-qwen-7b-gguf` |
| Family | `qwen` |
...
| Field | Value |
| --- | --- |
| Primary | `DeepSeek-R1-Distill-Qwen-7B-Q4_K_M.gguf` |
| Weight size | `4.36 GB` |
| Parameters | `7.0B` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `deepseek-ai/DeepSeek-R1-Distill-Qwen-7B` |
| Source | `unsloth/DeepSeek-R1-Distill-Qwen-7B-GGUF` |
| License | `mit` |
...
| Primary artifact | `DeepSeek-R1-Distill-Qwen-7B-Q4_K_M.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-reasoning-deepseek-r1-qwen-7b-gguf/
├── README.md                       # this card
...
└── DeepSeek-R1-Distill-Qwen-7B-Q4_K_M.gguf# primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Reasoning DeepSeek R1 Distill Qwen 7B GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
