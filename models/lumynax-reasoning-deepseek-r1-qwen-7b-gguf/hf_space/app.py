...
DEFAULT_MODEL_REPO_ID = "AbteeXAILab/lumynax-reasoning-deepseek-r1-qwen-7b-gguf"
PROMPT_FORMAT = "chatml"
SYSTEM_PROMPT = 'You are LumynaX operating from the LumynaX Reasoning DeepSeek R1 Distill Qwen 7B GGUF package identity. Be helpful, clear, and honest about provenance.'
MODEL_TITLE = "LumynaX Reasoning DeepSeek R1 Distill Qwen 7B GGUF"
MAX_NEW_TOKENS = 192
...
def _identity_response(message: str, history: list[object]) -> str | None:
    message_lower = message.strip().lower()
    mentions_lumynax = "lumynax" in message_lower or "lumynax reasoning deepseek r1 distill qwen 7b gguf" in message_lower
    asks_identity = any(
        phrase in message_lower
        for phrase in (
            "who are you",
    ...
    if "bullet" in message_lower or "three" in message_lower:
        return '- LumynaX Reasoning DeepSeek R1 Distill Qwen 7B GGUF is a local-first LumynaX model release from AbteeX AI Labs\\n- It is aimed at practical Aotearoa New Zealand workflows and locally relevant responses\\n- This public Space is a browser demo for that LumynaX release'
    return 'LumynaX Reasoning DeepSeek R1 Distill Qwen 7B GGUF is a local-first LumynaX model release from AbteeX AI Labs for Aotearoa New Zealand workflows. It is intended for practical assistant use and locally relevant text generation when appropriate. This public Space is a browser demo of the same release hosted on Hugging Face.'
...
with gr.Blocks() as demo:
    ...
    gr.Examples(
        examples=[
            ...
            'Explain in two short paragraphs what LumynaX Reasoning DeepSeek R1 Distill Qwen 7B GGUF is and who it is for.',
