...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Chat Yi-1.5 34B GGUF (multilingual) release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Chat Yi-1.5 34B GGUF (multilingual)</h1>
...
```bash
hf download AbteeXAILab/lumynax-chat-yi-15-34b-gguf --local-dir lumynax-chat-yi-15-34b-gguf
cd lumynax-chat-yi-15-34b-gguf
...
llm = Llama(model_path="Yi-1.5-34B-Chat-Q4_K_M.gguf", n_ctx=32768, n_threads=8, verbose=False)
...
```bash
llama-cli -m "Yi-1.5-34B-Chat-Q4_K_M.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-chat-yi-15-34b-gguf -f ollama/Modelfile
ollama run lumynax-chat-yi-15-34b-gguf
```

**Verify integrity before launch:**

```bash
sha256sum "Yi-1.5-34B-Chat-Q4_K_M.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "Yi-1.5-34B-Chat-Q4_K_M.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Chat Yi-1.5 34B GGUF (multilingual)` |
| Repository | `AbteeXAILab/lumynax-chat-yi-15-34b-gguf` |
| Family | `yi` |
...
| Field | Value |
| --- | --- |
| Primary | `Yi-1.5-34B-Chat-Q4_K_M.gguf` |
| Weight size | `—` |
| Parameters | `34B` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `01-ai/Yi-1.5-34B-Chat` |
| Source | `01-ai Yi` |
| License | `apache-2.0` |
...
| Primary artifact | `Yi-1.5-34B-Chat-Q4_K_M.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
└── Yi-1.5-34B-Chat-Q4_K_M.gguf     # primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Chat Yi-1.5 34B GGUF (multilingual) package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
