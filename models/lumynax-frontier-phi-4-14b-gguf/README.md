...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Frontier Phi-4 14B GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Frontier Phi-4 14B GGUF</h1>
...
```bash
hf download AbteeXAILab/lumynax-frontier-phi-4-14b-gguf --local-dir lumynax-frontier-phi-4-14b-gguf
cd lumynax-frontier-phi-4-14b-gguf
...
llm = Llama(model_path="phi-4-Q4_K_M.gguf", n_ctx=16384, n_threads=8, verbose=False)
...
```bash
llama-cli -m "phi-4-Q4_K_M.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-frontier-phi-4-14b-gguf -f ollama/Modelfile
ollama run lumynax-frontier-phi-4-14b-gguf
```

**Verify integrity before launch:**

```bash
sha256sum "phi-4-Q4_K_M.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "phi-4-Q4_K_M.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Frontier Phi-4 14B GGUF` |
| Repository | `AbteeXAILab/lumynax-frontier-phi-4-14b-gguf` |
| Family | `phi` |
...
| Primary artifact | `phi-4-Q4_K_M.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
└── phi-4-Q4_K_M.gguf               # primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Frontier Phi-4 14B GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
