...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Infused Phi-4 Text GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Infused Phi-4 Text GGUF</h1>
...
> `AbteeXAILab/lumynax-infused-phi-4-text-gguf` is a **complete LumynaX release package**: model artifact, `quickstart.py`, `requirements.txt`, `release_export_manifest.json`, `checksums.sha256`, license notice, and optional Ollama / Space scaffolds shipped as **one downloadable contract**. Clone whole, verify by checksum, and run close to the data it serves.
...
```bash
hf download AbteeXAILab/lumynax-infused-phi-4-text-gguf --local-dir lumynax-infused-phi-4-text-gguf
cd lumynax-infused-phi-4-text-gguf
...
llm = Llama(model_path="lumynax-infused-phi-4-text-gguf-f16.gguf", n_ctx=8192, n_threads=8, verbose=False)
...
```bash
llama-cli -m "lumynax-infused-phi-4-text-gguf-f16.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-infused-phi-4-text-gguf -f ollama/Modelfile
ollama run lumynax-infused-phi-4-text-gguf
```

**Verify integrity before launch:**

```bash
sha256sum "lumynax-infused-phi-4-text-gguf-f16.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "lumynax-infused-phi-4-text-gguf-f16.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Infused Phi-4 Text GGUF` |
| Repository | `AbteeXAILab/lumynax-infused-phi-4-text-gguf` |
| Family | `phi` |
...
| Field | Value |
| --- | --- |
| Primary | `lumynax-infused-phi-4-text-gguf-f16.gguf` |
| Weight size | `35.75 GB` |
...
| Primary artifact | `lumynax-infused-phi-4-text-gguf-f16.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-infused-phi-4-text-gguf/
├── README.md                       # this card
...
└── lumynax-infused-phi-4-text-gguf-f16.gguf# primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Infused Phi-4 Text GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
