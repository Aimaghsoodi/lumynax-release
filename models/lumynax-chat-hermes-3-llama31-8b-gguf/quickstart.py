"""
Lumynax Chat Hermes 3 Llama31 8B Gguf — LumynaX quickstart (clone & run).

This loads the GGUF that ships with this repo. No upstream HF call needed
once you've done `hf download AbteeXAILab/lumynax-chat-hermes-3-llama31-8b-gguf`.
...
PRIMARY = HERE / r"Hermes-3-Llama-3.1-8B-Q4_K_M.gguf"
...
def main():
    ...
    if not PRIMARY.exists():
        ...
        print(f"[lumynax] run: hf download AbteeXAILab/lumynax-chat-hermes-3-llama31-8b-gguf --local-dir <dir> first.", file=sys.stderr)
        sys.exit(2)
