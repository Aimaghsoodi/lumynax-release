...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Infused Zephyr 7B Beta GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Infused Zephyr 7B Beta GGUF</h1>
...
> `AbteeXAILab/lumynax-infused-zephyr-7b-beta-gguf` is a **complete LumynaX release package**: model artifact, `quickstart.py`, `requirements.txt`, `release_export_manifest.json`, `checksums.sha256`, license notice, and optional Ollama / Space scaffolds shipped as **one downloadable contract**. Clone whole, verify by checksum, and run close to the data it serves.
...
```bash
hf download AbteeXAILab/lumynax-infused-zephyr-7b-beta-gguf --local-dir lumynax-infused-zephyr-7b-beta-gguf
cd lumynax-infused-zephyr-7b-beta-gguf
...
llm = Llama(model_path="zephyr-7b-beta.Q4_K_M.gguf", n_ctx=8192, n_threads=8, verbose=False)
...
```bash
llama-cli -m "zephyr-7b-beta.Q4_K_M.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-infused-zephyr-7b-beta-gguf -f ollama/Modelfile
ollama run lumynax-infused-zephyr-7b-beta-gguf
```
...
```powershell
Get-FileHash -Algorithm SHA256 "zephyr-7b-beta.Q4_K_M.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Infused Zephyr 7B Beta GGUF` |
| Repository | `AbteeXAILab/lumynax-infused-zephyr-7b-beta-gguf` |
| Family | `lumynax` |
...
| Field | Value |
| --- | --- |
| Primary | `zephyr-7b-beta.Q4_K_M.gguf` |
| Weight size | `4.07 GB` |
| Parameters | `7.0B` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `HuggingFaceH4/zephyr-7b-beta` |
| Source | `TheBloke/zephyr-7B-beta-GGUF` |
| License | `mit` |
...
| Primary artifact | `zephyr-7b-beta.Q4_K_M.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-infused-zephyr-7b-beta-gguf/
├── README.md                       # this card
...
└── zephyr-7b-beta.Q4_K_M.gguf      # primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Infused Zephyr 7B Beta GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
