...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Infused Qwen2.5 7B Instruct GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Infused Qwen2.5 7B Instruct GGUF</h1>
...
```bash
hf download AbteeXAILab/lumynax-infused-qwen25-7b-instruct-gguf --local-dir lumynax-infused-qwen25-7b-instruct-gguf
cd lumynax-infused-qwen25-7b-instruct-gguf
...
llm = Llama(model_path="qwen2.5-7b-instruct-q3_k_m.gguf", n_ctx=32768, n_threads=8, verbose=False)
...
```bash
llama-cli -m "qwen2.5-7b-instruct-q3_k_m.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-infused-qwen25-7b-instruct-gguf -f ollama/Modelfile
ollama run lumynax-infused-qwen25-7b-instruct-gguf
```

**Verify integrity before launch:**

```bash
sha256sum "qwen2.5-7b-instruct-q3_k_m.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "qwen2.5-7b-instruct-q3_k_m.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Infused Qwen2.5 7B Instruct GGUF` |
| Repository | `AbteeXAILab/lumynax-infused-qwen25-7b-instruct-gguf` |
| Family | `qwen` |
...
| Field | Value |
| --- | --- |
| Primary | `qwen2.5-7b-instruct-q3_k_m.gguf` |
| Weight size | `3.55 GB` |
| Parameters | `7.0B` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `Qwen/Qwen2.5-7B-Instruct` |
...
| Primary artifact | `qwen2.5-7b-instruct-q3_k_m.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-infused-qwen25-7b-instruct-gguf/
├── README.md                       # this card
...
└── qwen2.5-7b-instruct-q3_k_m.gguf # primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Infused Qwen2.5 7B Instruct GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
