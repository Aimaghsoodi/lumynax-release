...
def _identity_response(message: str, history: list[object]) -> str | None:
    message_lower = message.strip().lower()
    mentions_lumynax = "lumynax" in message_lower or "lumynax infused qwen2.5 7b instruct gguf" in message_lower
    asks_identity = any(
        phrase in message_lower
        for phrase in (
            "who are you",
...
with gr.Blocks() as demo:
    ...
    gr.Examples(
        examples=[
            ...
            'Explain in two short paragraphs what LumynaX Infused Qwen2.5 7B Instruct GGUF is and who it is for.',
