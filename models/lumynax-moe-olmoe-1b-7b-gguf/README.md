...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX MoE OLMoE 1B-7B Instruct GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX MoE OLMoE 1B-7B Instruct GGUF</h1>
...
> `AbteeXAILab/lumynax-moe-olmoe-1b-7b-gguf` is a **complete LumynaX release package**: model artifact, `quickstart.py`, `requirements.txt`, `release_export_manifest.json`, `checksums.sha256`, license notice, and optional Ollama / Space scaffolds shipped as **one downloadable contract**. Clone whole, verify by checksum, and run close to the data it serves.
...
```bash
hf download AbteeXAILab/lumynax-moe-olmoe-1b-7b-gguf --local-dir lumynax-moe-olmoe-1b-7b-gguf
cd lumynax-moe-olmoe-1b-7b-gguf
...
llm = Llama(model_path="olmoe-1b-7b-0924-instruct-q4_k_m.gguf", n_ctx=8192, n_threads=8, verbose=False)
...
```bash
ollama create lumynax-moe-olmoe-1b-7b-gguf -f ollama/Modelfile
ollama run lumynax-moe-olmoe-1b-7b-gguf
```
...
| Field | Value |
| --- | --- |
| Release | `LumynaX MoE OLMoE 1B-7B Instruct GGUF` |
| Repository | `AbteeXAILab/lumynax-moe-olmoe-1b-7b-gguf` |
| Family | `olmo` |
...
| Source | `allenai/OLMoE-1B-7B-0924-Instruct-GGUF` |
| License | `apache-2.0` |
...
```text
You are LumynaX operating from the LumynaX MoE OLMoE 1B-7B Instruct GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
