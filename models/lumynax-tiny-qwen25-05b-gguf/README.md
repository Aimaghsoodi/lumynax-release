...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Tiny Qwen2.5 0.5B GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Tiny Qwen2.5 0.5B GGUF</h1>
...
<p align="center"><kbd>Quality: <b>1/5</b></kbd> &middot; <kbd>Lightweight: <b>4/5</b></kbd> &middot; <kbd>Sovereignty: <b>3/5</b></kbd> &middot; <kbd>Tools: <b>no</b></kbd> &middot; <kbd>JSON: <b>yes</b></kbd> &middot; <kbd>Context: <b>32768 tok</b></kbd></p>

---

## 📦 Executive Summary

> `AbteeXAILab/lumynax-tiny-qwen25-05b-gguf` is a **complete LumynaX release package**: model artifact, `quickstart.py`, `requirements.txt`, `release_export_manifest.json`, `checksums.sha256`, license notice, and optional Ollama / Space scaffolds shipped as **one downloadable contract**. Clone whole, verify by checksum, and run close to the data it serves.
...
```bash
hf download AbteeXAILab/lumynax-tiny-qwen25-05b-gguf --local-dir lumynax-tiny-qwen25-05b-gguf
cd lumynax-tiny-qwen25-05b-gguf
...
llm = Llama(model_path="lumynax-tiny-qwen25-05b-q4_k_m.gguf", n_ctx=32768, n_threads=8, verbose=False)
...
```bash
llama-cli -m "lumynax-tiny-qwen25-05b-q4_k_m.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-tiny-qwen25-05b-gguf -f ollama/Modelfile
ollama run lumynax-tiny-qwen25-05b-gguf
```

**Verify integrity before launch:**

```bash
sha256sum "lumynax-tiny-qwen25-05b-q4_k_m.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "lumynax-tiny-qwen25-05b-q4_k_m.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Tiny Qwen2.5 0.5B GGUF` |
| Repository | `AbteeXAILab/lumynax-tiny-qwen25-05b-gguf` |
| Family | `qwen` |
...
| Field | Value |
| --- | --- |
| Primary | `lumynax-tiny-qwen25-05b-q4_k_m.gguf` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `Qwen/Qwen2.5-0.5B-Instruct` |
...
| Primary artifact | `lumynax-tiny-qwen25-05b-q4_k_m.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
└── lumynax-tiny-qwen25-05b-q4_k_m.gguf# primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Tiny Qwen2.5 0.5B GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
