...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Reasoning InternLM3 8B Instruct GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Reasoning InternLM3 8B Instruct GGUF</h1>
...
```bash
hf download AbteeXAILab/lumynax-reasoning-internlm3-8b-gguf --local-dir lumynax-reasoning-internlm3-8b-gguf
cd lumynax-reasoning-internlm3-8b-gguf
...
llm = Llama(model_path="internlm3-8b-instruct-Q4_K_M.gguf", n_ctx=32768, n_threads=8, verbose=False)
...
```bash
llama-cli -m "internlm3-8b-instruct-Q4_K_M.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-reasoning-internlm3-8b-gguf -f ollama/Modelfile
ollama run lumynax-reasoning-internlm3-8b-gguf
```

**Verify integrity before launch:**

```bash
sha256sum "internlm3-8b-instruct-Q4_K_M.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "internlm3-8b-instruct-Q4_K_M.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Reasoning InternLM3 8B Instruct GGUF` |
| Repository | `AbteeXAILab/lumynax-reasoning-internlm3-8b-gguf` |
| Family | `internlm` |
...
| Field | Value |
| --- | --- |
| Primary | `internlm3-8b-instruct-Q4_K_M.gguf` |
| Weight size | `—` |
| Parameters | `8B` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `internlm/internlm3-8b-instruct` |
| Source | `InternLM` |
| License | `apache-2.0` |
...
| Primary artifact | `internlm3-8b-instruct-Q4_K_M.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-reasoning-internlm3-8b-gguf/
├── README.md                       # this card
...
└── internlm3-8b-instruct-Q4_K_M.gguf# primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Reasoning InternLM3 8B Instruct GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
