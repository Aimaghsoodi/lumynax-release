"""
Lumynax Multimodal Internvl3 78B Instruct — LumynaX quickstart (clone & run, multimodal safetensors).
...
def main():
    ...
    if not (HERE / "model.safetensors.index.json").exists():
        ...
        print(f"[lumynax] run: hf download AbteeXAILab/lumynax-multimodal-internvl3-78b-instruct --local-dir <dir> first.", file=sys.stderr)
        sys.exit(2)
