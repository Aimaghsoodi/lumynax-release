...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Infused Phi-3 Mini 4K Instruct GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Infused Phi-3 Mini 4K Instruct GGUF</h1>
...
> `AbteeXAILab/lumynax-infused-phi3-mini-4k-gguf` is a **complete LumynaX release package**: model artifact, `quickstart.py`, `requirements.txt`, `release_export_manifest.json`, `checksums.sha256`, license notice, and optional Ollama / Space scaffolds shipped as **one downloadable contract**. Clone whole, verify by checksum, and run close to the data it serves.
...
```bash
hf download AbteeXAILab/lumynax-infused-phi3-mini-4k-gguf --local-dir lumynax-infused-phi3-mini-4k-gguf
cd lumynax-infused-phi3-mini-4k-gguf
...
llm = Llama(model_path="Phi-3-mini-4k-instruct-q4.gguf", n_ctx=8192, n_threads=8, verbose=False)
...
```bash
llama-cli -m "Phi-3-mini-4k-instruct-q4.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-infused-phi3-mini-4k-gguf -f ollama/Modelfile
ollama run lumynax-infused-phi3-mini-4k-gguf
```
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Infused Phi-3 Mini 4K Instruct GGUF` |
| Repository | `AbteeXAILab/lumynax-infused-phi3-mini-4k-gguf` |
| Family | `phi` |
...
| Field | Value |
| --- | --- |
| Primary | `Phi-3-mini-4k-instruct-q4.gguf` |
| Weight size | `2.23 GB` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `microsoft/Phi-3-mini-4k-instruct` |
...
| Primary artifact | `Phi-3-mini-4k-instruct-q4.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-infused-phi3-mini-4k-gguf/
├── README.md                       # this card
...
└── Phi-3-mini-4k-instruct-q4.gguf  # primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Infused Phi-3 Mini 4K Instruct GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
