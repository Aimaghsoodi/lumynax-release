...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Infused SmolLM3 3B GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Infused SmolLM3 3B GGUF</h1>
...
> `AbteeXAILab/lumynax-infused-smollm3-3b-gguf` is a **complete LumynaX release package**: model artifact, `quickstart.py`, `requirements.txt`, `release_export_manifest.json`, `checksums.sha256`, license notice, and optional Ollama / Space scaffolds shipped as **one downloadable contract**. Clone whole, verify by checksum, and run close to the data it serves.
...
```bash
hf download AbteeXAILab/lumynax-infused-smollm3-3b-gguf --local-dir lumynax-infused-smollm3-3b-gguf
cd lumynax-infused-smollm3-3b-gguf
...
llm = Llama(model_path="SmolLM3-Q4_K_M.gguf", n_ctx=8192, n_threads=8, verbose=False)
...
```bash
llama-cli -m "SmolLM3-Q4_K_M.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-infused-smollm3-3b-gguf -f ollama/Modelfile
ollama run lumynax-infused-smollm3-3b-gguf
```

**Verify integrity before launch:**

```bash
sha256sum "SmolLM3-Q4_K_M.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "SmolLM3-Q4_K_M.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Infused SmolLM3 3B GGUF` |
| Repository | `AbteeXAILab/lumynax-infused-smollm3-3b-gguf` |
| Family | `smollm` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `HuggingFaceTB/SmolLM3-3B` |
...
| Primary artifact | `SmolLM3-Q4_K_M.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-infused-smollm3-3b-gguf/
├── README.md                       # this card
...
└── SmolLM3-Q4_K_M.gguf             # primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Infused SmolLM3 3B GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
