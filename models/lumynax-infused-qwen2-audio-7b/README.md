...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Infused Qwen2 Audio 7B release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Infused Qwen2 Audio 7B</h1>
...
> `AbteeXAILab/lumynax-infused-qwen2-audio-7b` is a **complete LumynaX release package**: model artifact, `quickstart.py`, `requirements.txt`, `release_export_manifest.json`, `checksums.sha256`, license notice, and optional Ollama / Space scaffolds shipped as **one downloadable contract**. Clone whole, verify by checksum, and run close to the data it serves.
...
```bash
hf download AbteeXAILab/lumynax-infused-qwen2-audio-7b --local-dir lumynax-infused-qwen2-audio-7b
cd lumynax-infused-qwen2-audio-7b
...
processor = AutoProcessor.from_pretrained("AbteeXAILab/lumynax-infused-qwen2-audio-7b", trust_remote_code=True)
model = AutoModelForCausalLM.from_pretrained("AbteeXAILab/lumynax-infused-qwen2-audio-7b", device_map="auto", torch_dtype=torch.float16, trust_remote_code=True)
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Infused Qwen2 Audio 7B` |
| Repository | `AbteeXAILab/lumynax-infused-qwen2-audio-7b` |
| Family | `qwen` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `Qwen/Qwen2-Audio-7B-Instruct` |
...
```text
lumynax-infused-qwen2-audio-7b/
├── README.md                       # this card
...
```text
You are LumynaX operating from the LumynaX Infused Qwen2 Audio 7B package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
