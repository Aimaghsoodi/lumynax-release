...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Reasoning GPT-OSS 20B GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Reasoning GPT-OSS 20B GGUF</h1>
...
```bash
hf download AbteeXAILab/lumynax-reasoning-gpt-oss-20b-gguf --local-dir lumynax-reasoning-gpt-oss-20b-gguf
cd lumynax-reasoning-gpt-oss-20b-gguf
...
llm = Llama(model_path="lumynax-reasoning-gpt-oss-20b-mxfp4.gguf", n_ctx=8192, n_threads=8, verbose=False)
...
```bash
llama-cli -m "lumynax-reasoning-gpt-oss-20b-mxfp4.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-reasoning-gpt-oss-20b-gguf -f ollama/Modelfile
ollama run lumynax-reasoning-gpt-oss-20b-gguf
```

**Verify integrity before launch:**

```bash
sha256sum "lumynax-reasoning-gpt-oss-20b-mxfp4.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "lumynax-reasoning-gpt-oss-20b-mxfp4.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Reasoning GPT-OSS 20B GGUF` |
| Repository | `AbteeXAILab/lumynax-reasoning-gpt-oss-20b-gguf` |
| Family | `lumynax` |
...
| Field | Value |
| --- | --- |
| Primary | `lumynax-reasoning-gpt-oss-20b-mxfp4.gguf` |
| Weight size | `11.28 GB` |
| Parameters | `20.0B` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `openai/gpt-oss-20b` |
...
| Primary artifact | `lumynax-reasoning-gpt-oss-20b-mxfp4.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-reasoning-gpt-oss-20b-gguf/
├── README.md                       # this card
...
└── lumynax-reasoning-gpt-oss-20b-mxfp4.gguf# primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Reasoning GPT-OSS 20B GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
