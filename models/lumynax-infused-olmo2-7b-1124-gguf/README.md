...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Infused OLMo 2 1124 7B Instruct GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Infused OLMo 2 1124 7B Instruct GGUF</h1>
...
> `AbteeXAILab/lumynax-infused-olmo2-7b-1124-gguf` is a **complete LumynaX release package**: model artifact, `quickstart.py`, `requirements.txt`, `release_export_manifest.json`, `checksums.sha256`, license notice, and optional Ollama / Space scaffolds shipped as **one downloadable contract**. Clone whole, verify by checksum, and run close to the data it serves.
...
```bash
hf download AbteeXAILab/lumynax-infused-olmo2-7b-1124-gguf --local-dir lumynax-infused-olmo2-7b-1124-gguf
cd lumynax-infused-olmo2-7b-1124-gguf
...
llm = Llama(model_path="olmo-2-1124-7B-instruct-Q4_K_M.gguf", n_ctx=8192, n_threads=8, verbose=False)
...
```bash
llama-cli -m "olmo-2-1124-7B-instruct-Q4_K_M.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-infused-olmo2-7b-1124-gguf -f ollama/Modelfile
ollama run lumynax-infused-olmo2-7b-1124-gguf
```

**Verify integrity before launch:**

```bash
sha256sum "olmo-2-1124-7B-instruct-Q4_K_M.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "olmo-2-1124-7B-instruct-Q4_K_M.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Infused OLMo 2 1124 7B Instruct GGUF` |
| Repository | `AbteeXAILab/lumynax-infused-olmo2-7b-1124-gguf` |
| Family | `olmo` |
...
| Field | Value |
| --- | --- |
| Primary | `olmo-2-1124-7B-instruct-Q4_K_M.gguf` |
| Weight size | `4.16 GB` |
| Parameters | `7.0B` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `allenai/OLMo-2-1124-7B-Instruct` |
| Source | `allenai/OLMo-2-1124-7B-Instruct-GGUF` |
| License | `apache-2.0` |
...
| Primary artifact | `olmo-2-1124-7B-instruct-Q4_K_M.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-infused-olmo2-7b-1124-gguf/
├── README.md                       # this card
...
└── olmo-2-1124-7B-instruct-Q4_K_M.gguf# primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Infused OLMo 2 1124 7B Instruct GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
