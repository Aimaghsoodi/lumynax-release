...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Infused Gemma E4B Text GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Infused Gemma E4B Text GGUF</h1>
...
> `AbteeXAILab/lumynax-infused-gemma-e4b-text-gguf` is a **complete LumynaX release package**: model artifact, `quickstart.py`, `requirements.txt`, `release_export_manifest.json`, `checksums.sha256`, license notice, and optional Ollama / Space scaffolds shipped as **one downloadable contract**. Clone whole, verify by checksum, and run close to the data it serves.
...
```bash
hf download AbteeXAILab/lumynax-infused-gemma-e4b-text-gguf --local-dir lumynax-infused-gemma-e4b-text-gguf
cd lumynax-infused-gemma-e4b-text-gguf
...
llm = Llama(model_path="lumynax-infused-gemma-e4b-text-gguf-q4_k_m.gguf", n_ctx=8192, n_threads=8, verbose=False)
...
```bash
llama-cli -m "lumynax-infused-gemma-e4b-text-gguf-q4_k_m.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-infused-gemma-e4b-text-gguf -f ollama/Modelfile
ollama run lumynax-infused-gemma-e4b-text-gguf
```

**Verify integrity before launch:**

```bash
sha256sum "lumynax-infused-gemma-e4b-text-gguf-q4_k_m.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "lumynax-infused-gemma-e4b-text-gguf-q4_k_m.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Infused Gemma E4B Text GGUF` |
| Repository | `AbteeXAILab/lumynax-infused-gemma-e4b-text-gguf` |
| Family | `gemma` |
...
| Field | Value |
| --- | --- |
| Primary | `lumynax-infused-gemma-e4b-text-gguf-q4_k_m.gguf` |
| Weight size | `4.97 GB` |
| Parameters | `4.0B` |
...
| Primary artifact | `lumynax-infused-gemma-e4b-text-gguf-q4_k_m.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-infused-gemma-e4b-text-gguf/
├── README.md                       # this card
...
└── lumynax-infused-gemma-e4b-text-gguf-q4_k_m.gguf# primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Infused Gemma E4B Text GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
