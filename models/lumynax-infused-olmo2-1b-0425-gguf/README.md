...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Infused OLMo 2 0425 1B Instruct GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Infused OLMo 2 0425 1B Instruct GGUF</h1>
...
> `AbteeXAILab/lumynax-infused-olmo2-1b-0425-gguf` is a **complete LumynaX release package**: model artifact, `quickstart.py`, `requirements.txt`, `release_export_manifest.json`, `checksums.sha256`, license notice, and optional Ollama / Space scaffolds shipped as **one downloadable contract**. Clone whole, verify by checksum, and run close to the data it serves.
...
```bash
hf download AbteeXAILab/lumynax-infused-olmo2-1b-0425-gguf --local-dir lumynax-infused-olmo2-1b-0425-gguf
cd lumynax-infused-olmo2-1b-0425-gguf
...
llm = Llama(model_path="OLMo-2-0425-1B-Instruct-Q4_K_M.gguf", n_ctx=8192, n_threads=8, verbose=False)
...
```bash
llama-cli -m "OLMo-2-0425-1B-Instruct-Q4_K_M.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-infused-olmo2-1b-0425-gguf -f ollama/Modelfile
ollama run lumynax-infused-olmo2-1b-0425-gguf
```

**Verify integrity before launch:**

```bash
sha256sum "OLMo-2-0425-1B-Instruct-Q4_K_M.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "OLMo-2-0425-1B-Instruct-Q4_K_M.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Infused OLMo 2 0425 1B Instruct GGUF` |
| Repository | `AbteeXAILab/lumynax-infused-olmo2-1b-0425-gguf` |
| Family | `olmo` |
...
| Field | Value |
| --- | --- |
| Primary | `OLMo-2-0425-1B-Instruct-Q4_K_M.gguf` |
| Weight size | `892.18 MB` |
| Parameters | `1.0B` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `allenai/OLMo-2-0425-1B-Instruct` |
| Source | `allenai/OLMo-2-0425-1B-Instruct-GGUF` |
| License | `apache-2.0` |
...
| Primary artifact | `OLMo-2-0425-1B-Instruct-Q4_K_M.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-infused-olmo2-1b-0425-gguf/
├── README.md                       # this card
...
└── OLMo-2-0425-1B-Instruct-Q4_K_M.gguf# primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Infused OLMo 2 0425 1B Instruct GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
