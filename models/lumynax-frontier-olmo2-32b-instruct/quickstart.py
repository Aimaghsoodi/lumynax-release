"""
Lumynax Frontier Olmo2 32B Instruct — LumynaX quickstart (clone & run, multimodal safetensors).
...
def main():
    ...
    if not (HERE / "model.safetensors.index.json").exists():
        ...
        print(f"[lumynax] run: hf download AbteeXAILab/lumynax-frontier-olmo2-32b-instruct --local-dir <dir> first.", file=sys.stderr)
        sys.exit(2)
