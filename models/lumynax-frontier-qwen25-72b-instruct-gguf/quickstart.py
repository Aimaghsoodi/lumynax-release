"""
Lumynax Frontier Qwen25 72B Instruct Gguf — LumynaX quickstart (clone & run).

This loads the GGUF that ships with this repo. No upstream HF call needed
once you've done `hf download AbteeXAILab/lumynax-frontier-qwen25-72b-instruct-gguf`.
...
PRIMARY = HERE / r"Qwen2.5-72B-Instruct-Q4_K_M.gguf"
...
def main():
    ...
    if not PRIMARY.exists():
        ...
        print(f"[lumynax] run: hf download AbteeXAILab/lumynax-frontier-qwen25-72b-instruct-gguf --local-dir <dir> first.", file=sys.stderr)
        sys.exit(2)
