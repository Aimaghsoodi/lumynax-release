...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Infused SmolLM 135M Instruct GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Infused SmolLM 135M Instruct GGUF</h1>
...
```bash
hf download AbteeXAILab/lumynax-infused-smollm-135m-gguf --local-dir lumynax-infused-smollm-135m-gguf
cd lumynax-infused-smollm-135m-gguf
...
llm = Llama(model_path="SmolLM-135M-Instruct.Q4_K_M.gguf", n_ctx=8192, n_threads=8, verbose=False)
...
```bash
llama-cli -m "SmolLM-135M-Instruct.Q4_K_M.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-infused-smollm-135m-gguf -f ollama/Modelfile
ollama run lumynax-infused-smollm-135m-gguf
```

**Verify integrity before launch:**

```bash
sha256sum "SmolLM-135M-Instruct.Q4_K_M.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "SmolLM-135M-Instruct.Q4_K_M.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Infused SmolLM 135M Instruct GGUF` |
| Repository | `AbteeXAILab/lumynax-infused-smollm-135m-gguf` |
| Family | `smollm` |
...
| Field | Value |
| --- | --- |
| Primary | `SmolLM-135M-Instruct.Q4_K_M.gguf` |
| Weight size | `100.57 MB` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `HuggingFaceTB/SmolLM-135M-Instruct` |
| Source | `MaziyarPanahi/SmolLM-135M-Instruct-GGUF` |
| License | `apache-2.0` |
...
| Primary artifact | `SmolLM-135M-Instruct.Q4_K_M.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-infused-smollm-135m-gguf/
├── README.md                       # this card
...
└── SmolLM-135M-Instruct.Q4_K_M.gguf# primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Infused SmolLM 135M Instruct GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
