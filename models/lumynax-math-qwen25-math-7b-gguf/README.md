...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Math Qwen2.5 Math 7B GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Math Qwen2.5 Math 7B GGUF</h1>
...
<p align="center"><kbd>Quality: <b>4/5</b></kbd> &middot; <kbd>Lightweight: <b>4/5</b></kbd> &middot; <kbd>Sovereignty: <b>3/5</b></kbd> &middot; <kbd>Tools: <b>no</b></kbd> &middot; <kbd>JSON: <b>yes</b></kbd> &middot; <kbd>Context: <b>4096 tok</b></kbd></p>

---

## 📦 Executive Summary

> `AbteeXAILab/lumynax-math-qwen25-math-7b-gguf` is a **complete LumynaX release package**: model artifact, `quickstart.py`, `requirements.txt`, `release_export_manifest.json`, `checksums.sha256`, license notice, and optional Ollama / Space scaffolds shipped as **one downloadable contract**. Clone whole, verify by checksum, and run close to the data it serves.
...
```bash
hf download AbteeXAILab/lumynax-math-qwen25-math-7b-gguf --local-dir lumynax-math-qwen25-math-7b-gguf
cd lumynax-math-qwen25-math-7b-gguf
...
llm = Llama(model_path="Qwen2.5-Math-7B-Instruct-Q4_K_M.gguf", n_ctx=4096, n_threads=8, verbose=False)
...
```bash
llama-cli -m "Qwen2.5-Math-7B-Instruct-Q4_K_M.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-math-qwen25-math-7b-gguf -f ollama/Modelfile
ollama run lumynax-math-qwen25-math-7b-gguf
```

**Verify integrity before launch:**

```bash
sha256sum "Qwen2.5-Math-7B-Instruct-Q4_K_M.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "Qwen2.5-Math-7B-Instruct-Q4_K_M.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Math Qwen2.5 Math 7B GGUF` |
| Repository | `AbteeXAILab/lumynax-math-qwen25-math-7b-gguf` |
| Family | `qwen` |
...
| Field | Value |
| --- | --- |
| Primary | `Qwen2.5-Math-7B-Instruct-Q4_K_M.gguf` |
| Weight size | `—` |
| Parameters | `7B` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `Qwen/Qwen2.5-Math-7B-Instruct` |
| Source | `Qwen2.5 Math` |
| License | `apache-2.0` |
...
| Primary artifact | `Qwen2.5-Math-7B-Instruct-Q4_K_M.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-math-qwen25-math-7b-gguf/
├── README.md                       # this card
...
└── Qwen2.5-Math-7B-Instruct-Q4_K_M.gguf# primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Math Qwen2.5 Math 7B GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
