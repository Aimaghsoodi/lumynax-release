...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Infused Mistral 7B Instruct v0.3 GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Infused Mistral 7B Instruct v0.3 GGUF</h1>
...
> `AbteeXAILab/lumynax-infused-mistral-7b-v03-gguf` is a **complete LumynaX release package**: model artifact, `quickstart.py`, `requirements.txt`, `release_export_manifest.json`, `checksums.sha256`, license notice, and optional Ollama / Space scaffolds shipped as **one downloadable contract**. Clone whole, verify by checksum, and run close to the data it serves.
...
```bash
hf download AbteeXAILab/lumynax-infused-mistral-7b-v03-gguf --local-dir lumynax-infused-mistral-7b-v03-gguf
cd lumynax-infused-mistral-7b-v03-gguf
...
llm = Llama(model_path="Mistral-7B-Instruct-v0.3-Q4_K_M.gguf", n_ctx=8192, n_threads=8, verbose=False)
...
```bash
llama-cli -m "Mistral-7B-Instruct-v0.3-Q4_K_M.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-infused-mistral-7b-v03-gguf -f ollama/Modelfile
ollama run lumynax-infused-mistral-7b-v03-gguf
```
...
```powershell
Get-FileHash -Algorithm SHA256 "Mistral-7B-Instruct-v0.3-Q4_K_M.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Infused Mistral 7B Instruct v0.3 GGUF` |
| Repository | `AbteeXAILab/lumynax-infused-mistral-7b-v03-gguf` |
| Family | `mistral` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `mistralai/Mistral-7B-Instruct-v0.3` |
...
| Primary artifact | `Mistral-7B-Instruct-v0.3-Q4_K_M.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-infused-mistral-7b-v03-gguf/
├── README.md                       # this card
...
└── Mistral-7B-Instruct-v0.3-Q4_K_M.gguf# primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Infused Mistral 7B Instruct v0.3 GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
