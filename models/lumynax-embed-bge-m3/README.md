...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Embed BGE M3 release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Embed BGE M3</h1>
...
```bash
hf download AbteeXAILab/lumynax-embed-bge-m3 --local-dir lumynax-embed-bge-m3
cd lumynax-embed-bge-m3
...
model = SentenceTransformer("AbteeXAILab/lumynax-embed-bge-m3", trust_remote_code=True)
...
```bash
sha256sum "merged_model/pytorch_model.bin"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "merged_model/pytorch_model.bin"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Embed BGE M3` |
| Repository | `AbteeXAILab/lumynax-embed-bge-m3` |
| Family | `embedding` |
...
| Field | Value |
| --- | --- |
| Primary | `merged_model/pytorch_model.bin` |
| Weight size | `2.12 GB` |
| Parameters | `see manifest` |
| Quality rank | `4` (1 best) |
...
| Signal | Reading |
| --- | --- |
| Quality rank | `4` (1 = strongest in family) |
...
| Primary artifact | `merged_model/pytorch_model.bin` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
└── merged_model/pytorch_model.bin  # primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Embed BGE M3 package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
