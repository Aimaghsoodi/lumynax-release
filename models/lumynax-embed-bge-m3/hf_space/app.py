...
DEFAULT_MODEL_REPO_ID = "AbteeXAILab/lumynax-embed-bge-m3"
...
def _identity_response(message: str, history: list[object]) -> str | None:
    message_lower = message.strip().lower()
    mentions_lumynax = "lumynax" in message_lower or "lumynax embed bge m3" in message_lower
    asks_identity = any(
        phrase in message_lower
        for phrase in (
            "who are you",
    ...
    if "bullet" in message_lower or "three" in message_lower:
        return '- LumynaX Embed BGE M3 is a local-first LumynaX model release from AbteeX AI Labs\\n- It is aimed at practical Aotearoa New Zealand workflows and locally relevant responses\\n- This public Space is a browser demo for that LumynaX release'
    return 'LumynaX Embed BGE M3 is a local-first LumynaX model release from AbteeX AI Labs for Aotearoa New Zealand workflows. It is intended for practical assistant use and locally relevant text generation when appropriate. This public Space is a browser demo of the same release hosted on Hugging Face.'
