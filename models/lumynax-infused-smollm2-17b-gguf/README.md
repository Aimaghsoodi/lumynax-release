...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Infused SmolLM2 1.7B Instruct GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Infused SmolLM2 1.7B Instruct GGUF</h1>
...
```bash
hf download AbteeXAILab/lumynax-infused-smollm2-17b-gguf --local-dir lumynax-infused-smollm2-17b-gguf
cd lumynax-infused-smollm2-17b-gguf
...
llm = Llama(model_path="smollm2-1.7b-instruct-q4_k_m.gguf", n_ctx=8192, n_threads=8, verbose=False)
...
```bash
llama-cli -m "smollm2-1.7b-instruct-q4_k_m.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-infused-smollm2-17b-gguf -f ollama/Modelfile
ollama run lumynax-infused-smollm2-17b-gguf
```

**Verify integrity before launch:**

```bash
sha256sum "smollm2-1.7b-instruct-q4_k_m.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "smollm2-1.7b-instruct-q4_k_m.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Infused SmolLM2 1.7B Instruct GGUF` |
| Repository | `AbteeXAILab/lumynax-infused-smollm2-17b-gguf` |
| Family | `smollm` |
...
| Field | Value |
| --- | --- |
| Primary | `smollm2-1.7b-instruct-q4_k_m.gguf` |
| Weight size | `1006.71 MB` |
| Parameters | `1.7B` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `HuggingFaceTB/SmolLM2-1.7B-Instruct` |
...
| Primary artifact | `smollm2-1.7b-instruct-q4_k_m.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-infused-smollm2-17b-gguf/
├── README.md                       # this card
...
└── smollm2-1.7b-instruct-q4_k_m.gguf# primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Infused SmolLM2 1.7B Instruct GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
