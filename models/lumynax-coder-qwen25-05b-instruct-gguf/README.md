...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Coder Qwen2.5 0.5B Instruct GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Coder Qwen2.5 0.5B Instruct GGUF</h1>
...
```bash
hf download AbteeXAILab/lumynax-coder-qwen25-05b-instruct-gguf --local-dir lumynax-coder-qwen25-05b-instruct-gguf
cd lumynax-coder-qwen25-05b-instruct-gguf
...
llm = Llama(model_path="qwen2.5-coder-0.5b-instruct-q4_k_m.gguf", n_ctx=32768, n_threads=8, verbose=False)
...
```bash
llama-cli -m "qwen2.5-coder-0.5b-instruct-q4_k_m.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-coder-qwen25-05b-instruct-gguf -f ollama/Modelfile
ollama run lumynax-coder-qwen25-05b-instruct-gguf
```

**Verify integrity before launch:**

```bash
sha256sum "qwen2.5-coder-0.5b-instruct-q4_k_m.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "qwen2.5-coder-0.5b-instruct-q4_k_m.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Coder Qwen2.5 0.5B Instruct GGUF` |
| Repository | `AbteeXAILab/lumynax-coder-qwen25-05b-instruct-gguf` |
| Family | `qwen` |
...
| Field | Value |
| --- | --- |
| Primary | `qwen2.5-coder-0.5b-instruct-q4_k_m.gguf` |
| Weight size | `468.64 MB` |
| Parameters | `0.5B` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `Qwen/Qwen2.5-Coder-0.5B-Instruct` |
...
| Primary artifact | `qwen2.5-coder-0.5b-instruct-q4_k_m.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-coder-qwen25-05b-instruct-gguf/
├── README.md                       # this card
...
└── qwen2.5-coder-0.5b-instruct-q4_k_m.gguf# primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Coder Qwen2.5 0.5B Instruct GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
