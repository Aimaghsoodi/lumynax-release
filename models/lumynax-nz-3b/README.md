...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX NZ 3B V1 release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX NZ 3B V1</h1>
...
> `AbteeXAILab/lumynax-nz-3b` is a **complete LumynaX release package**: model artifact, `quickstart.py`, `requirements.txt`, `release_export_manifest.json`, `checksums.sha256`, license notice, and optional Ollama / Space scaffolds shipped as **one downloadable contract**. Clone whole, verify by checksum, and run close to the data it serves.
...
```bash
hf download AbteeXAILab/lumynax-nz-3b --local-dir lumynax-nz-3b
cd lumynax-nz-3b
...
tokenizer = AutoTokenizer.from_pretrained("AbteeXAILab/lumynax-nz-3b", trust_remote_code=True)
model = AutoModelForCausalLM.from_pretrained("AbteeXAILab/lumynax-nz-3b", device_map="auto", trust_remote_code=True)
...
```bash
sha256sum "merged_model/model-00001-of-00055.safetensors"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "merged_model/model-00001-of-00055.safetensors"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX NZ 3B V1` |
| Repository | `AbteeXAILab/lumynax-nz-3b` |
| Family | `lumynax` |
...
| Field | Value |
| --- | --- |
| Primary | `merged_model/model-00001-of-00055.safetensors` |
| Weight size | `11.50 GB` |
| Parameters | `3.0B` |
...
| Primary artifact | `merged_model/model-00001-of-00055.safetensors` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
└── merged_model/model-00001-of-00055.safetensors# primary artifact
```
...
```text
You are LumynaX operating from the LumynaX NZ 3B V1 package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
