...
DEFAULT_MODEL_REPO_ID = "AbteeXAILab/lumynax-nz-3b"
PROMPT_FORMAT = "chatml"
SYSTEM_PROMPT = 'You are LumynaX NZ 3B V1, a local-first LumynaX model release from AbteeX AI Labs for Aotearoa New Zealand workflows. If asked what LumynaX is, explain that this is a LumynaX model and demo release from AbteeX AI Labs. Do not describe LumynaX as hardware, a printer, or an unrelated product. Do not identify yourself as ChatGPT, Qwen, Gemma, or any other standalone assistant. If asked about lineage, explain that you are a LumynaX release built on upstream open weights. Default to a helpful multi-sentence answer unless the user asks for brevity. Keep answers practical, clear, and locally relevant when appropriate.'
MODEL_TITLE = "LumynaX NZ 3B V1"
MAX_NEW_TOKENS = 192
...
def _identity_response(message: str, history: list[object]) -> str | None:
    message_lower = message.strip().lower()
    mentions_lumynax = "lumynax" in message_lower or "lumynax nz 3b v1" in message_lower
    asks_identity = any(
        phrase in message_lower
        for phrase in (
            "who are you",
    ...
    if "bullet" in message_lower or "three" in message_lower:
        return '- LumynaX NZ 3B V1 is a local-first LumynaX model release from AbteeX AI Labs\\n- It is aimed at practical Aotearoa New Zealand workflows and locally relevant responses\\n- This public Space is a browser demo for that LumynaX release'
    return 'LumynaX NZ 3B V1 is a local-first LumynaX model release from AbteeX AI Labs for Aotearoa New Zealand workflows. It is intended for practical assistant use and locally relevant text generation when appropriate. This public Space is a browser demo of the same release hosted on Hugging Face.'
...
def chat(message: str, history: list[object]) -> tuple[str, list[object]]:
    ...
    try:
        ...
        next_history = history + [
            {"role": "user", "content": message},
            {"role": "assistant", "content": f"Demo runtime error: {type(exc).__name__}: {exc}"},
        ]
        return "", _append_history(history, message, f"Demo runtime error: {type(exc).__name__}: {exc}")
...
with gr.Blocks() as demo:
    gr.Markdown(
        ...
        "The first request after a cold start can take a minute or two while the model loads on CPU."
    )
