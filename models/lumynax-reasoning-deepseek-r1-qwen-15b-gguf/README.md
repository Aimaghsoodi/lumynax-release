...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Reasoning DeepSeek R1 Distill Qwen 1.5B GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Reasoning DeepSeek R1 Distill Qwen 1.5B GGUF</h1>
...
```bash
hf download AbteeXAILab/lumynax-reasoning-deepseek-r1-qwen-15b-gguf --local-dir lumynax-reasoning-deepseek-r1-qwen-15b-gguf
cd lumynax-reasoning-deepseek-r1-qwen-15b-gguf
...
llm = Llama(model_path="DeepSeek-R1-Distill-Qwen-1.5B-Q4_K_M.gguf", n_ctx=8192, n_threads=8, verbose=False)
...
```bash
llama-cli -m "DeepSeek-R1-Distill-Qwen-1.5B-Q4_K_M.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-reasoning-deepseek-r1-qwen-15b-gguf -f ollama/Modelfile
ollama run lumynax-reasoning-deepseek-r1-qwen-15b-gguf
```
...
```powershell
Get-FileHash -Algorithm SHA256 "DeepSeek-R1-Distill-Qwen-1.5B-Q4_K_M.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Reasoning DeepSeek R1 Distill Qwen 1.5B GGUF` |
| Repository | `AbteeXAILab/lumynax-reasoning-deepseek-r1-qwen-15b-gguf` |
| Family | `qwen` |
...
| Field | Value |
| --- | --- |
| Primary | `DeepSeek-R1-Distill-Qwen-1.5B-Q4_K_M.gguf` |
| Weight size | `1.04 GB` |
| Parameters | `1.5B` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B` |
...
| Primary artifact | `DeepSeek-R1-Distill-Qwen-1.5B-Q4_K_M.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-reasoning-deepseek-r1-qwen-15b-gguf/
├── README.md                       # this card
...
└── DeepSeek-R1-Distill-Qwen-1.5B-Q4_K_M.gguf# primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Reasoning DeepSeek R1 Distill Qwen 1.5B GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
