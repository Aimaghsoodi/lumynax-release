"""
LumynaX Embed Nomic v2 MoE (modern retrieval) — LumynaX quickstart.
...
documented in the README (LumynaX Embed Nomic v2 MoE (modern retrieval)).
...
LumynaX package repo: https://huggingface.co/AbteeXAILab/lumynax-embed-nomic-v2-moe
Upstream weights:     https://huggingface.co/nomic-ai/nomic-embed-text-v2-moe
"""
...
def _run_hf(prompt: str, interactive: bool):
    ...
    print("[lumynax] Loading nomic-ai/nomic-embed-text-v2-moe. This is a >100B MoE — multi-GPU or accelerate offload recommended.")
    tok = AutoTokenizer.from_pretrained("nomic-ai/nomic-embed-text-v2-moe", trust_remote_code=True)
    model = AutoModelForCausalLM.from_pretrained(
        "nomic-ai/nomic-embed-text-v2-moe", device_map="auto", torch_dtype="auto", trust_remote_code=True
    )
    def chat(user):
        messages = [
