...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Multimodal Kimi VL A3B Thinking release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Multimodal Kimi VL A3B Thinking</h1>
...
> `AbteeXAILab/lumynax-multimodal-kimi-vl-a3b-thinking` is a **complete LumynaX release package**: model artifact, `quickstart.py`, `requirements.txt`, `release_export_manifest.json`, `checksums.sha256`, license notice, and optional Ollama / Space scaffolds shipped as **one downloadable contract**. Clone whole, verify by checksum, and run close to the data it serves.
...
```bash
hf download AbteeXAILab/lumynax-multimodal-kimi-vl-a3b-thinking --local-dir lumynax-multimodal-kimi-vl-a3b-thinking
cd lumynax-multimodal-kimi-vl-a3b-thinking
...
processor = AutoProcessor.from_pretrained("AbteeXAILab/lumynax-multimodal-kimi-vl-a3b-thinking", trust_remote_code=True)
model = AutoModelForCausalLM.from_pretrained("AbteeXAILab/lumynax-multimodal-kimi-vl-a3b-thinking", device_map="auto", torch_dtype=torch.float16, trust_remote_code=True)
...
```bash
sha256sum "lumynax-multimodal-kimi-vl-a3b-thinking-q4_k_m.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "lumynax-multimodal-kimi-vl-a3b-thinking-q4_k_m.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Multimodal Kimi VL A3B Thinking` |
| Repository | `AbteeXAILab/lumynax-multimodal-kimi-vl-a3b-thinking` |
| Family | `kimi` |
...
| Prompt format | `llama_cpp_multimodal` |
...
| Field | Value |
| --- | --- |
| Primary | `lumynax-multimodal-kimi-vl-a3b-thinking-q4_k_m.gguf` |
| Weight size | `10.39 GB` |
| Parameters | `3.0B / 3.0B active` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `moonshotai/Kimi-VL-A3B-Thinking-2506` |
| Source | `ggml-org/Kimi-VL-A3B-Thinking-2506-GGUF` |
| License | `mit` |
...
| Primary artifact | `lumynax-multimodal-kimi-vl-a3b-thinking-q4_k_m.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-multimodal-kimi-vl-a3b-thinking/
├── README.md                       # this card
...
└── lumynax-multimodal-kimi-vl-a3b-thinking-q4_k_m.gguf# primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Multimodal Kimi VL A3B Thinking package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
