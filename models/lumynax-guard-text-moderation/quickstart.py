"""
Lumynax Guard Text Moderation — LumynaX quickstart (clone & run, multimodal safetensors).
...
def main():
    ...
    if not (HERE / "model.safetensors").exists():
        ...
        print(f"[lumynax] run: hf download AbteeXAILab/lumynax-guard-text-moderation --local-dir <dir> first.", file=sys.stderr)
        sys.exit(2)
