...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Guard Text Moderation release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Guard Text Moderation</h1>
...
> `AbteeXAILab/lumynax-guard-text-moderation` is a **complete LumynaX release package**: model artifact, `quickstart.py`, `requirements.txt`, `release_export_manifest.json`, `checksums.sha256`, license notice, and optional Ollama / Space scaffolds shipped as **one downloadable contract**. Clone whole, verify by checksum, and run close to the data it serves.
...
tokenizer = AutoTokenizer.from_pretrained("AbteeXAILab/lumynax-guard-text-moderation", trust_remote_code=True)
model = AutoModelForCausalLM.from_pretrained("AbteeXAILab/lumynax-guard-text-moderation", device_map="auto", trust_remote_code=True)
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Guard Text Moderation` |
| Repository | `AbteeXAILab/lumynax-guard-text-moderation` |
| Family | `roberta` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `KoalaAI/Text-Moderation` |
| Source | `KoalaAI Text Moderation` |
| License | `openrail` |
...
```text
You are LumynaX operating from the LumynaX Guard Text Moderation package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
