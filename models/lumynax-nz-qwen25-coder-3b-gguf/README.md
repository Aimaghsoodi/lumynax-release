...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX NZ Qwen2.5 Coder 3B GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX NZ Qwen2.5 Coder 3B GGUF</h1>
...
```bash
hf download AbteeXAILab/lumynax-nz-qwen25-coder-3b-gguf --local-dir lumynax-nz-qwen25-coder-3b-gguf
cd lumynax-nz-qwen25-coder-3b-gguf
...
llm = Llama(model_path="lumynax-nz-qwen25-coder-3b-q4_k_m.gguf", n_ctx=32768, n_threads=8, verbose=False)
...
```bash
llama-cli -m "lumynax-nz-qwen25-coder-3b-q4_k_m.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-nz-qwen25-coder-3b-gguf -f ollama/Modelfile
ollama run lumynax-nz-qwen25-coder-3b-gguf
```

**Verify integrity before launch:**

```bash
sha256sum "lumynax-nz-qwen25-coder-3b-q4_k_m.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "lumynax-nz-qwen25-coder-3b-q4_k_m.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX NZ Qwen2.5 Coder 3B GGUF` |
| Repository | `AbteeXAILab/lumynax-nz-qwen25-coder-3b-gguf` |
| Family | `qwen` |
...
| Field | Value |
| --- | --- |
| Primary | `lumynax-nz-qwen25-coder-3b-q4_k_m.gguf` |
| Weight size | `1.96 GB` |
| Parameters | `3.0B` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `Qwen/Qwen2.5-Coder-3B-Instruct` |
...
| Primary artifact | `lumynax-nz-qwen25-coder-3b-q4_k_m.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-nz-qwen25-coder-3b-gguf/
├── README.md                       # this card
...
└── lumynax-nz-qwen25-coder-3b-q4_k_m.gguf# primary artifact
```
...
```text
You are LumynaX operating from the LumynaX NZ Qwen2.5 Coder 3B GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
