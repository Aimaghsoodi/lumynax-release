...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Long-Context Qwen2.5 7B Instruct 1M GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Long-Context Qwen2.5 7B Instruct 1M GGUF</h1>
...
> `AbteeXAILab/lumynax-longctx-qwen25-7b-1m-gguf` is a **complete LumynaX release package**: model artifact, `quickstart.py`, `requirements.txt`, `release_export_manifest.json`, `checksums.sha256`, license notice, and optional Ollama / Space scaffolds shipped as **one downloadable contract**. Clone whole, verify by checksum, and run close to the data it serves.
...
llm = Llama(model_path="Qwen2.5-7B-Instruct-1M-Q4_K_M.gguf", n_ctx=1048576, n_threads=8, verbose=False)
...
```bash
llama-cli -m "Qwen2.5-7B-Instruct-1M-Q4_K_M.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-longctx-qwen25-7b-1m-gguf -f ollama/Modelfile
ollama run lumynax-longctx-qwen25-7b-1m-gguf
```

**Verify integrity before launch:**

```bash
sha256sum "Qwen2.5-7B-Instruct-1M-Q4_K_M.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "Qwen2.5-7B-Instruct-1M-Q4_K_M.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Long-Context Qwen2.5 7B Instruct 1M GGUF` |
| Repository | `AbteeXAILab/lumynax-longctx-qwen25-7b-1m-gguf` |
| Family | `qwen` |
...
| Field | Value |
| --- | --- |
| Primary | `Qwen2.5-7B-Instruct-1M-Q4_K_M.gguf` |
| Weight size | `—` |
| Parameters | `7B` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `Qwen/Qwen2.5-7B-Instruct-1M` |
| Source | `Qwen2.5 1M` |
| License | `apache-2.0` |
...
| Primary artifact | `Qwen2.5-7B-Instruct-1M-Q4_K_M.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-longctx-qwen25-7b-1m-gguf/
├── README.md                       # this card
...
└── Qwen2.5-7B-Instruct-1M-Q4_K_M.gguf# primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Long-Context Qwen2.5 7B Instruct 1M GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
