"""
Lumynax Multimodal Pixtral Large 124B — LumynaX quickstart (clone & run, multimodal safetensors).

Loads the local safetensors shards in this repo via transformers.
Requires significant VRAM (200+ GB VRAM).
...
def main():
    ...
    args = p.parse_args()
    if not (HERE / "consolidated.safetensors.index.json").exists():
        print(f"[lumynax] weight index missing in {HERE}", file=sys.stderr)
        print(f"[lumynax] run: hf download AbteeXAILab/lumynax-multimodal-pixtral-large-124b --local-dir <dir> first.", file=sys.stderr)
        sys.exit(2)
