"""
LumynaX Doc Nougat Base (academic PDF -> markdown) — LumynaX quickstart.
...
documented in the README (LumynaX Doc Nougat Base (academic PDF -> markdown)).
...
LumynaX package repo: https://huggingface.co/AbteeXAILab/lumynax-doc-nougat-base
Upstream weights:     https://huggingface.co/facebook/nougat-base
"""
...
def _run_hf(prompt: str, interactive: bool):
    ...
    print("[lumynax] Loading facebook/nougat-base. This is a >100B MoE — multi-GPU or accelerate offload recommended.")
    tok = AutoTokenizer.from_pretrained("facebook/nougat-base", trust_remote_code=True)
    model = AutoModelForCausalLM.from_pretrained(
        "facebook/nougat-base", device_map="auto", torch_dtype="auto", trust_remote_code=True
    )
    def chat(user):
        messages = [
