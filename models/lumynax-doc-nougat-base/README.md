...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Doc Nougat Base (academic PDF -> markdown) release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Doc Nougat Base (academic PDF -> markdown)</h1>
...
> `AbteeXAILab/lumynax-doc-nougat-base` is a **complete LumynaX release package**: model artifact, `quickstart.py`, `requirements.txt`, `release_export_manifest.json`, `checksums.sha256`, license notice, and optional Ollama / Space scaffolds shipped as **one downloadable contract**. Clone whole, verify by checksum, and run close to the data it serves.
...
```bash
hf download AbteeXAILab/lumynax-doc-nougat-base --local-dir lumynax-doc-nougat-base
cd lumynax-doc-nougat-base
...
tokenizer = AutoTokenizer.from_pretrained("AbteeXAILab/lumynax-doc-nougat-base", trust_remote_code=True)
model = AutoModelForCausalLM.from_pretrained("AbteeXAILab/lumynax-doc-nougat-base", device_map="auto", trust_remote_code=True)
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Doc Nougat Base (academic PDF -> markdown)` |
| Repository | `AbteeXAILab/lumynax-doc-nougat-base` |
| Family | `nougat` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `facebook/nougat-base` |
| Source | `Meta Nougat` |
| License | `cc-by-4.0` |
...
```text
You are LumynaX operating from the LumynaX Doc Nougat Base (academic PDF -> markdown) package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
