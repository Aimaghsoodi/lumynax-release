...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Reasoning DeepSeek Distill Text GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Reasoning DeepSeek Distill Text GGUF</h1>
...
```bash
hf download AbteeXAILab/lumynax-reasoning-deepseek-distill-text-gguf --local-dir lumynax-reasoning-deepseek-distill-text-gguf
cd lumynax-reasoning-deepseek-distill-text-gguf
...
llm = Llama(model_path="lumynax-reasoning-deepseek-distill-text-gguf-f16.gguf", n_ctx=8192, n_threads=8, verbose=False)
...
```bash
llama-cli -m "lumynax-reasoning-deepseek-distill-text-gguf-f16.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-reasoning-deepseek-distill-text-gguf -f ollama/Modelfile
ollama run lumynax-reasoning-deepseek-distill-text-gguf
```

**Verify integrity before launch:**

```bash
sha256sum "lumynax-reasoning-deepseek-distill-text-gguf-f16.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "lumynax-reasoning-deepseek-distill-text-gguf-f16.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Reasoning DeepSeek Distill Text GGUF` |
| Repository | `AbteeXAILab/lumynax-reasoning-deepseek-distill-text-gguf` |
| Family | `deepseek` |
...
| Field | Value |
| --- | --- |
| Primary | `lumynax-reasoning-deepseek-distill-text-gguf-f16.gguf` |
| Weight size | `35.89 GB` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `deepseek-ai/DeepSeek-R1-Distill-Qwen-14B` |
...
| Primary artifact | `lumynax-reasoning-deepseek-distill-text-gguf-f16.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-reasoning-deepseek-distill-text-gguf/
├── README.md                       # this card
...
└── lumynax-reasoning-deepseek-distill-text-gguf-f16.gguf# primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Reasoning DeepSeek Distill Text GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
...
| **Upstream / base** | `deepseek-ai/DeepSeek-R1-Distill-Qwen-14B` |
| **Source** | `not applicable` |
| **License metadata** | `deepseek-license` |
