...
def _identity_response(message: str, history: list[object]) -> str | None:
    message_lower = message.strip().lower()
    mentions_lumynax = "lumynax" in message_lower or "lumynax infused smollm2 360m instruct gguf" in message_lower
    asks_identity = any(
        phrase in message_lower
        for phrase in (
            "who are you",
...
with gr.Blocks() as demo:
    ...
    gr.Examples(
        examples=[
            ...
            'Explain in two short paragraphs what LumynaX Infused SmolLM2 360M Instruct GGUF is and who it is for.',
