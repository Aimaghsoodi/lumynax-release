...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Infused Qwen3 14B GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Infused Qwen3 14B GGUF</h1>
...
<p align="center"><kbd>Quality: <b>4/5</b></kbd> &middot; <kbd>Lightweight: <b>0/5</b></kbd> &middot; <kbd>Sovereignty: <b>3/5</b></kbd> &middot; <kbd>Tools: <b>no</b></kbd> &middot; <kbd>JSON: <b>yes</b></kbd> &middot; <kbd>Context: <b>32768 tok</b></kbd></p>

---

## 📦 Executive Summary

> `AbteeXAILab/lumynax-infused-qwen3-14b-gguf` is a **complete LumynaX release package**: model artifact, `quickstart.py`, `requirements.txt`, `release_export_manifest.json`, `checksums.sha256`, license notice, and optional Ollama / Space scaffolds shipped as **one downloadable contract**. Clone whole, verify by checksum, and run close to the data it serves.
...
llm = Llama(model_path="Qwen3-14B-Q4_K_M.gguf", n_ctx=32768, n_threads=8, verbose=False)
...
```bash
llama-cli -m "Qwen3-14B-Q4_K_M.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-infused-qwen3-14b-gguf -f ollama/Modelfile
...
```bash
sha256sum "Qwen3-14B-Q4_K_M.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "Qwen3-14B-Q4_K_M.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Infused Qwen3 14B GGUF` |
| Repository | `AbteeXAILab/lumynax-infused-qwen3-14b-gguf` |
| Family | `qwen` |
...
| Primary artifact | `Qwen3-14B-Q4_K_M.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
└── Qwen3-14B-Q4_K_M.gguf           # primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Infused Qwen3 14B GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
