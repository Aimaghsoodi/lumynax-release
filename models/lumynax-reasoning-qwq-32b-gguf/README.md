...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Reasoning QwQ-32B GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Reasoning QwQ-32B GGUF</h1>
...
<p align="center"><kbd>Quality: <b>5/5</b></kbd> &middot; <kbd>Lightweight: <b>3/5</b></kbd> &middot; <kbd>Sovereignty: <b>3/5</b></kbd> &middot; <kbd>Tools: <b>yes</b></kbd> &middot; <kbd>JSON: <b>yes</b></kbd> &middot; <kbd>Context: <b>131072 tok</b></kbd></p>

---

## 📦 Executive Summary

> `AbteeXAILab/lumynax-reasoning-qwq-32b-gguf` is a **complete LumynaX release package**: model artifact, `quickstart.py`, `requirements.txt`, `release_export_manifest.json`, `checksums.sha256`, license notice, and optional Ollama / Space scaffolds shipped as **one downloadable contract**. Clone whole, verify by checksum, and run close to the data it serves.
...
llm = Llama(model_path="qwq-32b-q4_k_m.gguf", n_ctx=131072, n_threads=8, verbose=False)
...
```bash
llama-cli -m "qwq-32b-q4_k_m.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-reasoning-qwq-32b-gguf -f ollama/Modelfile
ollama run lumynax-reasoning-qwq-32b-gguf
```

**Verify integrity before launch:**

```bash
sha256sum "qwq-32b-q4_k_m.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "qwq-32b-q4_k_m.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Reasoning QwQ-32B GGUF` |
| Repository | `AbteeXAILab/lumynax-reasoning-qwq-32b-gguf` |
| Family | `qwen` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `Qwen/QwQ-32B` |
| Source | `Qwen QwQ reasoning` |
| License | `apache-2.0` |
...
| Primary artifact | `qwq-32b-q4_k_m.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-reasoning-qwq-32b-gguf/
├── README.md                       # this card
...
└── qwq-32b-q4_k_m.gguf             # primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Reasoning QwQ-32B GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
