...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Infused Qwen3 30B A3B GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Infused Qwen3 30B A3B GGUF</h1>
...
```bash
hf download AbteeXAILab/lumynax-infused-qwen3-30b-a3b-gguf --local-dir lumynax-infused-qwen3-30b-a3b-gguf
cd lumynax-infused-qwen3-30b-a3b-gguf
...
llm = Llama(model_path="lumynax-infused-qwen3-30b-a3b-q4_k_m.gguf", n_ctx=32768, n_threads=8, verbose=False)
...
```bash
llama-cli -m "lumynax-infused-qwen3-30b-a3b-q4_k_m.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-infused-qwen3-30b-a3b-gguf -f ollama/Modelfile
ollama run lumynax-infused-qwen3-30b-a3b-gguf
```

**Verify integrity before launch:**

```bash
sha256sum "lumynax-infused-qwen3-30b-a3b-q4_k_m.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "lumynax-infused-qwen3-30b-a3b-q4_k_m.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Infused Qwen3 30B A3B GGUF` |
| Repository | `AbteeXAILab/lumynax-infused-qwen3-30b-a3b-gguf` |
| Family | `qwen` |
...
| Field | Value |
| --- | --- |
| Primary | `lumynax-infused-qwen3-30b-a3b-q4_k_m.gguf` |
| Weight size | `17.28 GB` |
| Parameters | `30.0B / 3.0B active` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `Qwen/Qwen3-30B-A3B` |
...
| Primary artifact | `lumynax-infused-qwen3-30b-a3b-q4_k_m.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-infused-qwen3-30b-a3b-gguf/
├── README.md                       # this card
...
└── lumynax-infused-qwen3-30b-a3b-q4_k_m.gguf# primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Infused Qwen3 30B A3B GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
