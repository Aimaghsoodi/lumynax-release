...
from .download import run_pulled_model
from .gateway import route_chat_payload
from .ops import cache_report, inspect_hardware
...
def load_gateway_config(path: Path | None = None) -> dict[str, Any]:
    ...
    config = _expand_gateway_env(config)
...
def handle_gateway_request(
    ...
    if method == "GET" and path in {"/v1/local/cache", "/v1/local/models"}:
        cache_root = Path(str(config["cache_dir"])) if str(config.get("cache_dir") or "") else None
        return 200, cache_report(models, cache_root)
    if method == "GET" and path == "/v1/local/health":
        cache_root = Path(str(config["cache_dir"])) if str(config.get("cache_dir") or "") else None
        return 200, {
            "ok": True,
            "mode": config["mode"],
            "queue": {"active": 0, "queued": 0},
            "cache": cache_report(models, cache_root),
            "hardware": inspect_hardware(cache_root),
        }
...
def chat_completion_gateway(
    ...
    if str(config.get("mode") or "").lower() in {"local_live", "live_local"} or bool(config.get("local_live")):
        return _local_chat_completion(payload, selected, models, config, route_result, receipt)
    ...
    return _proxy_chat_completion(payload, selected, backend, config, route_result, receipt)
...
def smoke_gateway(
    *,
    ...
    chat_payload = load_json_mapping(default_chat_request_path())
...
def _local_chat_completion(
    ...
    prompt = str(route_result.get("routing_request", {}).get("prompt") or "")
    if not prompt:
        return 422, {"ok": False, "error": "empty_prompt", "receipt": receipt, **route_result}
    cache_dir = Path(str(config["cache_dir"])) if str(config.get("cache_dir") or "") else None
    try:
        result = run_pulled_model(
            ...
            pull=bool(config.get("pull_missing", False)),
            max_tokens=int(payload.get("max_tokens") or config.get("max_tokens") or 192),
            temperature=float(payload.get("temperature") or config.get("temperature") or 0.2),
            context_tokens=_optional_int(payload.get("context_tokens") or config.get("context_tokens")),
            threads=_optional_int(payload.get("threads") or config.get("threads")),
        )
...
def _proxy_chat_completion(
    ...
    backend_type = str(backend.get("type") or "marama_chat_http")
...
def _expand_gateway_env(value: Any) -> Any:
    if isinstance(value, str):
        return os.path.expandvars(value)
    if isinstance(value, dict):
        return {str(key): _expand_gateway_env(item) for key, item in value.items()}
    if isinstance(value, list):
        return [_expand_gateway_env(item) for item in value]
    return value
